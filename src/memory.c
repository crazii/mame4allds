											  ...
											  mem_read_handler handler, int install_at_beginning)
{
	...
	if (readport[cpu] == 0)
	{
		readport[cpu] = (struct IOReadPort *) malloc(readport_size[cpu]);
	}
	else
	{
		readport[cpu] = (struct IOReadPort *) realloc(readport[cpu], readport_size[cpu]);
	}
											   ...
											   mem_write_handler handler, int install_at_beginning)
{
	...
	if (writeport[cpu] == 0)
	{
		writeport[cpu] = (struct IOWritePort *) malloc(writeport_size[cpu]);
	}
	else
	{
		writeport[cpu] = (struct IOWritePort *) realloc(writeport[cpu], writeport_size[cpu]);
	}
