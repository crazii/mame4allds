...
#if !defined(GP2X) && !defined(WIZ) && !defined(NDS)
	if (options.vector_width == 0) options.vector_width = 640;
	if (options.vector_height == 0) options.vector_height = 480;
#else
	if (options.vector_width == 0) options.vector_width = 256;
	if (options.vector_height == 0) options.vector_height = 192;
#endif
		...
		if (init_machine() == 0)
		{
			...
			else if (!bailing)
			{
				...
				printf("Unable to start machine emulation\n");
                                nds_flush_video_down();while(nds_joystick_read()) { nds_timer_delay(150); } nds_joystick_press();
			}
...
int run_machine(void)
{
    ...
    printf("start the video hardware\n");
    if (drv->vh_start == 0 || (*drv->vh_start)() == 0)      /* start the video hardware */
    {
      printf("start the audio hardware\n");
