...
static int ecd_read(ZIP* zip) {
	...
	while (1) {
		...
		//	return -1;
		//}
    fseek(zip->fp, zip->length - buf_length, SEEK_SET);
		...
		//if (fread( buf, 1, buf_length, zip->fp ) != 1) {
...
ZIP* openzip(const char* zipfile) {
  ...
  fseek(zip->fp, zip->offset_to_start_of_cent_dir, SEEK_SET);
	...
	//if (fread(zip->cd, 1, zip->size_of_cent_dir, zip->fp)!=1) {
...
int readcompresszip(ZIP* zip, struct zipent* ent, char* data) {
	...
	//if (fread(data, 1, ent->compressed_size, zip->fp)!=1) {
...
static ZIP* cache_openzip(const char* zipfile) {
	...
	for(i=0;i<ZIP_CACHE_MAX;++i) {
		if (zip_cache_map[i] && strcasecmp(zip_cache_map[i]->zip,zipfile)==0) {
			/* found */
			unsigned j;
...
int /* error */ load_zipped_file (const char* zipfile, const char* filename, unsigned char** buf, unsigned int* length) {
	...
	while (readzip(zip)) {
		...
		if (equal_filename(ent->name, filename) ||
				(ent->crc32 && !strcasecmp(crc, filename)))
		{
