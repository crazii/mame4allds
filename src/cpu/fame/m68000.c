...
void FAME_SetRegs(FAME_Regs *src);
void FAME_GetRegs(FAME_Regs *dst);
void MC_MUSASHI000_SetRegs(FAME_Regs *src);
void MC_MUSASHI000_GetRegs(FAME_Regs *dst);
...
void FAME_Cause_Interrupt(int level)
{
#ifndef MAME4ALL_GOLD
	if (cpu_getactivecpu() || using_always_musashi)
...
void FAME_Clear_Pending_Interrupts(void)
{
#ifndef MAME4ALL_GOLD
	if (cpu_getactivecpu() || using_always_musashi)
