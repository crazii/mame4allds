...
#define SCREEN8 nds_screen15
#define SCREEN16 nds_screen15
#define FLUSH_CACHE  	{   ds2_flipScreen(UP_SCREEN, 0); nds_screen15 = up_screen_addr; }
...
{ \
  ...
  register unsigned short *u32dest = (unsigned short *)DEST; \
	...
	while (n) \
	{ \
		/* *u32dest++ = (nds_palette[*(src+3)] << 16) | nds_palette[*src];*/ \
    *u32dest++ = nds_palette[*src]; \
    src+=2; \
    *u32dest++ = nds_palette[*src]; \
    src+=2; \
		/* src+=4; */ \
...
{ \
  ...
  register unsigned short *u32dest = (unsigned short *)DEST; \
	...
	while (n) \
	{ \
		/* *u32dest++ = (nds_palette[*(src++ + 1)] << 16) | nds_palette[*src++]; */ \
		/* *u32dest++ = (nds_palette[*(src++ + 1)] << 16) | nds_palette[*src++]; */ \
		/* *u32dest++ = (nds_palette[*(src++ + 1)] << 16) | nds_palette[*src++]; */ \
		/* *u32dest++ = (nds_palette[*(src++ + 1)] << 16) | nds_palette[*src++]; */ \
    *u32dest++ = nds_palette[*src++]; \
...
{
    return nds_video_color15((nds_video_getr15(color1)+nds_video_getr15(color2))>>1,(nds_video_getg15(color1)+nds_video_getg15(color2))>>1,(nds_video_getb15(color1)+nds_video_getb15(color2))>>1);
}
		...
		for (x = 0; x < gfx_display_columns; )
		{
			...
			if (ISDIRTY(x,y))
			{
				...
				for (h = 0; ((h < 16) && ((y + h) < gfx_display_lines)); h++)
				{
					...
					clut8to16(address0, lb0, nds_palette, w);
					lb0 += width;
					address0 += gfx_width;
				}
			}
...
INLINE void blitscreen_dirty0_color8_noscale(struct osd_bitmap *bitmap)
{
	...
	do
	{
		...
		clut8to16(address, lb, nds_palette, columns);
		lb+=width;
		address+=gfx_width;
		y--;
	}
...
{
	unsigned short *buffer_scr = (unsigned short *)(SCREEN8+gfx_xoffset+(NDS_SCREEN_WIDTH*gfx_yoffset));
	...
	if (gfx_width>NDS_SCREEN_WIDTH)
	{
		/* Scale Down */
		step=NDS_SCREEN_WIDTH/(gfx_width-NDS_SCREEN_WIDTH);
		do {
			x=NDS_SCREEN_WIDTH; i=step;
			do {
				*buffer_scr++=(nds_palette[*buffer_mem++]);
				x--; i--;
	...
	{
		/* Scale Up */
		step=NDS_SCREEN_WIDTH/(NDS_SCREEN_WIDTH-gfx_width);
		do {
			x=NDS_SCREEN_WIDTH; i=1;
			do {
				i--;
				if (i) { *buffer_scr++=(nds_palette[*buffer_mem++]); }
				else { *buffer_scr++=(nds_palette[*buffer_mem]); i=step; }
				x--;
			} while (x);						
			buffer_mem+=buffer_mem_offset;		
			y--;
...
INLINE void blitscreen_dirty0_color8_fitscale_merge1(struct osd_bitmap *bitmap)
{
  ...
  for (h = 0; h < lines; h++)
  {
    ...
    {
      register unsigned int srcPixel = nds_palette[*srcX++];
...
{
	unsigned short *buffer_scr = SCREEN16 + gfx_xoffset + (NDS_SCREEN_WIDTH*gfx_yoffset);
