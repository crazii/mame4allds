...
static void updatekeyboard(void)
{
	...
	key[KEY_1]= (ExKey1 & NDS_START) && !(ExKey1 & NDS_SELECT);
	/* Coin A == Select Button */
	key[KEY_5]= (ExKey1 & NDS_START) && (ExKey1 & NDS_SELECT);

	/* MAME Menu */
	key[KEY_TAB]=(ExKey1 & NDS_B) && (ExKey1 & NDS_SELECT);
	/* Volume */
	key[KEY_TILDE]=(ExKey1 & NDS_X) && (ExKey1 & NDS_SELECT);
	/* Enter */
	key[KEY_ENTER]=(ExKey1 & NDS_B) && !(ExKey1 & NDS_SELECT);
	/* Space */
	key[KEY_SPACE]=(ExKey1 & NDS_X) && !(ExKey1 & NDS_SELECT);;
	/* Ctrl */
	key[KEY_LCONTROL]=(ExKey1 & NDS_Y) && !(ExKey1 & NDS_SELECT);
	/* Exit */
	key[KEY_ESC]=((ExKey1 & NDS_L) && (ExKey1 & NDS_R) && (ExKey1 & NDS_START));
	/* Pause */
	key[KEY_P]=(ExKey1 & NDS_Y) && (ExKey1 & NDS_SELECT);
	/* FPS */
	key[KEY_F11]=(ExKey1 & NDS_R) && (ExKey1 & NDS_SELECT);
	/* Profiler */
	key[KEY_LSHIFT]=(ExKey1 & NDS_L) && (ExKey1 & NDS_SELECT);

        /* Test if stylus is used */
        if (ExKey1 & NDS_TOUCH) {
          unsigned long xy=nds_stylus_read();
          unsigned int x=(xy & 0xffff0000)>>16;
          unsigned int y=(xy & 0xffff);

          // 31,98 to 115,133 -> insert coins
          if ((x>31) && (x<115) && (y>98) && (y<133)) {
            key[KEY_5]=1; 
          }
          // 141,98 to 225,134 -> snapshot
          else if ((x>141) && (x<225) && (y>98) && (y<133)) {
            key[KEY_F12]=1; 
          }
          // 31,143 to 115,177 -> fps
          else if ((x>31) && (x<115) && (y>143) && (y<177)) {
            key[KEY_F9]=1; 
          }
          // 141,143 to 225,177 -> exit
          else if ((x>141) && (x<225) && (y>143) && (y<177)) {
            key[KEY_ESC]=1; 
          }
        }
}
...
static int is_joy_button_pressed (int button, int ExKey)
{
	switch (button)
	{
		case 0: return ExKey & NDS_A; break;
		case 1: return ExKey & NDS_B; break;
		case 2: return ExKey & NDS_X; break;
		case 3: return ExKey & NDS_Y; break;
		case 4: return ExKey & NDS_L; break;
		case 5: return ExKey & NDS_R; break;
		default: break;
	}
	return 0; 
}
...
static int is_joy_axis_pressed (int axis, int dir, int ExKey)
{
	...
	if (!rotate_controls)
	{
		...
		if (axis==0)
		{
			switch (dir)
			{
				case 1: return ExKey & NDS_LEFT; break;
				case 2: return ExKey & NDS_RIGHT; break;
		...
		else
		{
			switch (dir)
			{
				case 1: return ExKey & NDS_UP; break;
				case 2: return ExKey & NDS_DOWN; break;
				default: return 0; break;
			}
		}
	}
	else
	{
		...
		else
		{
			switch (dir)
			{
				case 1: return ExKey & NDS_RIGHT; break;
				case 2: return ExKey & NDS_LEFT; break;
				default: return 0; break;
			}
		}
	}
...
int osd_is_joy_pressed(int joycode)
{
	...
	{
		case MOUSE_BUTTON(1):
			return ExKey1 & NDS_A; break;
		case MOUSE_BUTTON(2):
			return ExKey1 & NDS_B; break;
		case MOUSE_BUTTON(3):
			return ExKey1 & NDS_X; break;
	}
