...
//ALEK
                {
                  FILE *f;
                  char name[200],name_ref[200];
                  f=fopen("/MAME/MAMELIST.TXT","w");

		  fprintf(f,"%s","+----------------------------------+-------+-------+-------+-------+----------+--------------+----------+\n");
		  fprintf(f,"%s","|                                  |       |Correct|       |Screen | Internal |     Rom      |          |\n");
		  fprintf(f,"%s","| Game Name                        |Working|Colors | Sound | Flip  |   Name   |     Name     |    CRC   |\n");
		  fprintf(f,"%s","+----------------------------------+-------+-------+-------+-------+----------+--------------+----------+\n");
		  ...
		  while (drivers[i]) {
		    ...
		    else sprintf(name_ref,"%s ",name);

		    /* print the additional description only if we are listing clones */
		    if (listclones) {
                      if (strchr(drivers[i]->description,'(')) strcat(name_ref,strchr(drivers[i]->description,'('));
		    }
		    sprintf(name,"| %-33.33s",name_ref);
                    fprintf(f,"%s",name);
		    if (drivers[i]->flags & (GAME_NOT_WORKING | GAME_UNEMULATED_PROTECTION)) {
                      const struct GameDriver *maindrv;
		      int foundworking;
                      if (drivers[i]->clone_of && !(drivers[i]->clone_of->flags & NOT_A_DRIVER)) maindrv = drivers[i]->clone_of;
                      else maindrv = drivers[i];
                      foundworking = 0;
		      j = 0;
                      while (drivers[j]) {
			...
			j++;
                      ...
                      if (foundworking) fprintf(f,"%s","| No(1) ");
		      else fprintf(f,"%s","|   No  ");
		    }
		    else fprintf(f,"%s","|  Yes  ");
		    if (drivers[i]->flags & GAME_WRONG_COLORS) fprintf(f,"%s","|   No  ");
		    else if (drivers[i]->flags & GAME_IMPERFECT_COLORS) fprintf(f,"%s","| Close ");
		    else  fprintf(f,"%s","|  Yes  ");
...
#endif
		      if (drivers[i]->flags & GAME_NO_SOUND) fprintf(f,"%s","|   No  ");
                      else 
                      if (drivers[i]->flags & GAME_IMPERFECT_SOUND) {
                        if (samplenames) fprintf(f,"%s","|Part(2)");
			else fprintf(f,"%s","|Partial");
		      }
                      else {
			if (samplenames) fprintf(f,"%","| Yes(2)");
			else fprintf(f,"%s","|  Yes  ");
		      }
		    }
		    if (drivers[i]->flags & GAME_NO_COCKTAIL) fprintf(f,"%s","|   No  ");
		    else fprintf(f,"%s","|  Yes  ");
		    sprintf(name,"| %-8s | %-12s | %-8s |\n",drivers[i]->name,"","");
                    fprintf(f,"%s",name);
