	...
	while (romp->name || romp->offset || romp->length)
	{
		...
		while (romp->length)
		{
			...
			else
			{
				...
				{
					sprintf (&buf[strlen(buf)], "%-12s NOT FOUND alk\n",name);
					fatalerror = 1;
				}
...
static struct GameSample *read_wav_sample(void *f)
{
	...
	result = (struct GameSample *) malloc(sizeof(struct GameSample) + length); // ALEK
	if (result == NULL)
		return NULL;
...
/* if first samplename is *dir, looks for samples into "basename" first, then "dir" */
{
	...
	if ((samples = (struct GameSamples *) malloc(sizeof(struct GameSamples) + (i-1)*sizeof(struct GameSample))) == 0) // ALEK
		return 0;
...
void freesamples(struct GameSamples *samples)
{
	...
	for (i = 0;i < samples->total;i++)
		if (samples->sample[i]) free(samples->sample[i]);
