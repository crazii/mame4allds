...
{
	if (palette_used_colors) free(palette_used_colors);
	palette_used_colors = old_used_colors = just_remapped = new_palette = palette_dirty = 0;
	if (pen_visiblecount) free(pen_visiblecount);
	pen_visiblecount = 0;
	if (game_palette) free(game_palette);
	game_palette = 0;
	if (palette_map) free(palette_map);
	palette_map = 0;
	if (Machine->game_colortable) free(Machine->game_colortable);
	Machine->game_colortable = 0;
	if (Machine->remapped_colortable) free(Machine->remapped_colortable);
	Machine->remapped_colortable = 0;
	if (shrinked_pens) free(shrinked_pens);
	shrinked_pens = 0;
	if (shrinked_palette) free(shrinked_palette);
	shrinked_palette = 0;
	if (Machine->pens)  free(Machine->pens);
	Machine->pens = 0;
	if (palette_shadow_table) free(palette_shadow_table);
